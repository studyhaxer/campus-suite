        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                @can('create', \App\Models\FeeInvoice::class)
                    <button wire:click="generateInvoices"
                        wire:confirm="Generate invoices for this month for all active students without one?"
            ...
            <div class="mb-4 grid grid-cols-1 sm:grid-cols-4 gap-3">
                ...
                <input type="text" wire:model.live.debounce.300ms="search"
                    placeholder="Search student or admission #" class="rounded-md border-gray-300 shadow-sm text-sm">
                                ...
                                <tr wire:key="invoice-{{ $invoice->id }}">
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                        ...
                                        <div class="text-xs text-gray-400 font-mono">
                                            {{ $invoice->student->admission_number }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ $invoice->student->classSection?->fullName ?? '—' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ number_format($invoice->amount, 2) }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                    ...
                                    <td class="px-6 py-4 text-sm text-right">
                                        @can('create', \App\Models\FeePayment::class)
                                            @if ($invoice->status !== 'paid')
                                                <button wire:click="openPay({{ $invoice->id }})"
                                                    class="text-indigo-600 hover:text-indigo-800">Collect Payment</button>
                                            @endif
                                        @endcan
                                    </td>
                                </tr>
                ...
                <div class="p-6">
                    ...
                    <form wire:submit="recordPayment" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="payNotes"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        </div>
                        <div class="flex justify-end gap-3 pt-4 border-t">
                            ...
                            <button type="submit"
                                class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700">Record
                                Payment</button>
                        </div>
                    </form>
                </div>
