                            ...
                            @forelse ($campuses as $campus)
                                <tr wire:key="campus-{{ $campus->id }}">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <span class="px-2 py-1 text-xs rounded-full {{ $campus->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                            {{ $campus->is_active ? 'Active' : 'Inactive' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-right space-x-3">
                                        <button wire:click="openEdit({{ $campus->id }})" class="text-indigo-600 hover:text-indigo-800">Edit</button>
                                        <button wire:click="toggleActive({{ $campus->id }})" wire:confirm="Are you sure?" class="text-gray-500 hover:text-gray-700">
                    ...
                    <form wire:submit="save" class="space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="region" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Contact Phone</label>
                                <input type="text" wire:model="contact_phone" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Contact Email</label>
                                <input type="email" wire:model="contact_email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                @error('contact_email') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Timezone</label>
                                <input type="text" wire:model="timezone" placeholder="e.g. Asia/Karachi" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Currency</label>
                                <input type="text" wire:model="currency" maxlength="3" placeholder="e.g. USD" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm uppercase">
                                @error('currency') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Academic Year</label>
                                <input type="text" wire:model="academic_year" placeholder="e.g. 2026-2027" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Pay Cycle</label>
                                <select wire:model="pay_cycle" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
